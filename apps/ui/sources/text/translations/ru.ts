...
export const ru: TranslationStructure = {
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "Без группировки",
      flatSubtitle: "Все сессии одним списком, без заголовков групп",
    },
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: 'Фоновая служба вошла под другим аккаунтом',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `Приложение использует ${activeRelayUrl}, но фоновая служба на этом компьютере вошла под другим аккаунтом.`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: 'Корень рабочей области',
        ...
        moveSheetTitle: ({ item }: { item: string }) => `Переместить ${item}`,
        moveSheetDestinationLabel: 'Место назначения',
        moveSheetSubmit: 'Переместить',
        moveSheetSearchPlaceholder: 'Поиск папок…',
        moveSheetEmpty: 'Нет доступных мест назначения',
        moveSheetDestinations: 'Места назначения',
        moveSheetDisabledDescendant: 'Нельзя переместить в себя или во вложенную папку.',
        moveSheetDisabledMaxDepth: 'Будет превышена максимальная глубина папок.',
        moveSheetDisabledCurrent: 'Уже находится здесь.',
        moveSheetDisabledUnavailable: 'Это место назначения недоступно.',
        dragHandleA11yLabel: 'Маркер перетаскивания',
        dragA11yPickedUp: ({ item }: { item: string }) => `${item}: взято.`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `${item} перемещено рядом с ${destination}.`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `${item} перемещено в ${destination}.`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `${item} перемещено на уровень ${destination}.`,
        dragA11yCancelled: ({ item }: { item: string }) => `Перемещение ${item} отменено.`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `Не удалось переместить ${item}: ${reason}`,
        dragA11yBlockedDescendantCycle: 'место назначения находится внутри перемещаемой папки',
        dragA11yBlockedLeafCannotBeParent: 'сессии не могут содержать другие элементы',
        dragA11yBlockedMaxDepth: 'достигнута максимальная глубина папок',
        dragA11yBlockedSamePosition: 'уже в этой позиции',
        dragA11yBlockedWorkspaceScope: 'место назначения в другой рабочей области',
        dragA11yBlockedNoTarget: 'место назначения не выбрано',
        dragA11yBlockedDirectSession: 'прямые сессии нельзя перемещать в папки',
        dragA11yBlockedFeatureDisabled: 'папки сессий не включены',
        ...
        selectionSelectedCount: ({ count }: { count: number }) => `Выбрано сессий: ${count}`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => `Выбрано сессий: ${count}`,
        ...
        selectionProgress: ({ completed, total }: { completed: number; total: number }) => `Выполнено ${completed} из ${total}`,
        selectionCancelRunningA11yLabel: 'Отменить действие с выбранными сессиями',
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `Успешно: ${succeeded}, ошибок: ${failed}, пропущено: ${skipped}`,
        selectionDismissResultA11yLabel: 'Скрыть результат действия с выбранными сессиями',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `${action}: выбрано сессий — ${count}?`,
        selectionConfirmA11yLabel: ({ action }: { action: string }) => `Подтвердить: ${action}`,
        selectionMoveSheetSourceLabel: ({ count }: { count: number }) => `Выбрано сессий: ${count}`,
        orderingMode: {
            title: 'Порядок сессий',
            description: 'Выберите ручной порядок или стабильную сортировку по дате.',
            custom: 'Свой порядок',
            created: 'По дате создания',
            updated: 'По последней активности',
        },
    attentionSectionTitle: 'Требует внимания',
    activeSectionTitle: 'Активные',
    inactiveSectionTitle: 'Неактивные',
    pinnedSectionTitle: 'Закреплённые',
    reminders: {
...
settingsSession: {
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `Проверяем, что этот компьютер отвечает на ${relay}.`,
    blockedTitle: 'Настройка остановлена',
    blockedStatusFallback: 'Настройку не удалось завершить. Ваша учётная запись и сессии не затронуты.',
    blockedServiceConflictStatus: 'Сначала нужно разобраться с другой фоновой службой на этом компьютере. Ничего не изменено — откройте «Подробности», чтобы увидеть, с какой.',
    blockedConsentDeclinedStatus: 'Настройка остановилась до того, как что-либо изменить. Фоновая служба, которая уже была здесь, осталась ровно такой же.',
    blockedPairingDeclinedStatus: 'Этот компьютер не был одобрен для подключения. Ничего не изменено.',
    ...
    canceledStatus: 'Ничего не осталось наполовину сделанным. Повторный запуск настройки продолжит с этого места.',
    ...
    relayMoveTitle: 'Перенести фоновую службу на этот Relay?',
    relayMoveBody: ({ relay }: { relay: string }) => `\u0424\u043e\u043d\u043e\u0432\u0430\u044f \u0441\u043b\u0443\u0436\u0431\u0430 \u044d\u0442\u043e\u0433\u043e \u043a\u043e\u043c\u043f\u044c\u044e\u0442\u0435\u0440\u0430 \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442 \u043d\u0430 \u0434\u0440\u0443\u0433\u043e\u043c Relay. \u041f\u0435\u0440\u0435\u043d\u0435\u0441\u0442\u0438 \u0435\u0451 \u043d\u0430 ${relay}?`,
    relayMoveConfirm: 'Перенести',
    relayMoveKeep: 'Оставить как есть',
    relayMoveAlways: 'Всегда переносить',
