...
export const zhHans: TranslationStructure = {
  ...
  settingsPets: {
    ...
    visibilityModeAttentionOrActive: '等你处理或进行中时',
    visibilityModeAttentionOnly: '仅等你处理时',
  },
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "不分组",
      flatSubtitle: "所有会话在一个列表中，不显示分组标题",
    },
  ...
  newSession: {
    ...
    startNewSessionInFolder: "在此文件夹新建任务",
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: '你的后台服务登录的是另一个账号',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `应用正在使用 ${activeRelayUrl}，但这台电脑的后台服务登录的是另一个账号。`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: '工作区根目录',
        ...
        moveSheetEmpty: '没有可移动到的位置',
        moveSheetDestinations: '可选位置',
        moveSheetDisabledDescendant: '不能移动到自身或其子文件夹中。',
        moveSheetDisabledMaxDepth: '这会超出文件夹层级上限。',
        moveSheetDisabledCurrent: '已在此位置。',
        moveSheetDisabledUnavailable: '此位置不可用。',
        dragHandleA11yLabel: '拖动手柄',
        dragA11yPickedUp: ({ item }: { item: string }) => `已拿起 ${item}。`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `已将 ${item} 移到 ${destination} 旁边。`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `已将 ${item} 移入 ${destination}。`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `已将 ${item} 移到 ${destination}。`,
        dragA11yCancelled: ({ item }: { item: string }) => `已取消移动 ${item}。`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `无法移动 ${item}：${reason}`,
        dragA11yBlockedDescendantCycle: '目标位于被移动的文件夹内',
        dragA11yBlockedLeafCannotBeParent: '会话不能包含其他项目',
        dragA11yBlockedMaxDepth: '已达到文件夹层级上限',
        dragA11yBlockedSamePosition: '已在该位置',
        dragA11yBlockedWorkspaceScope: '目标位于其他工作区',
        dragA11yBlockedNoTarget: '未选择目标',
        dragA11yBlockedDirectSession: '直连会话不能移入文件夹',
        dragA11yBlockedFeatureDisabled: '未启用会话文件夹',
        dragA11yBlockedUnsupportedItem: '此项目不能移入文件夹',
        dragA11yBlockedDateOrderingMode: '会话顺序由当前日期排序控制。',
        selectionSelectedCount: ({ count }: { count: number }) => `已选择 ${count} 个会话`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => `已选择 ${count} 个会话`,
        ...
        selectionKeepInAttention: '保留在「等你处理」',
        selectionRemoveFromAttention: '移出「等你处理」',
        ...
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `成功 ${succeeded} 个，失败 ${failed} 个，跳过 ${skipped} 个`,
        selectionDismissResultA11yLabel: '关闭所选会话操作结果',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `要${action}已选的 ${count} 个会话吗？`,
    ...
    attentionSectionTitle: '等你处理',
    activeSectionTitle: '活动中',
    inactiveSectionTitle: '非活动',
  ...
  directSessions: {
    ...
    keepInAttention: "保留在「等你处理」",
    keepInAttentionSubtitle: "即使已读，也会留在这里",
    removeFromAttention: "移出「等你处理」",
...
settingsSession: {
    newSessionDraftEntry: {
	    ...
	    sessionList: {
         ...
         agentActivityCountDisabledSubtitle: '列表行中不显示代理数量',
	        ...
	        workingStatusAnimatedTextDisabledSubtitle: '任务进行时显示固定的“进行中…”',
	        attentionPromotionModeTitle: '等你处理的任务',
	        attentionPromotionModeSubtitle: '等你批准、回复或查看的任务放在哪里',
	        attentionPromotionModeOffTitle: '保留在常规位置',
	        attentionPromotionModeOffSubtitle: '保持列表现有的分组和排序',
	        attentionPromotionModeGlobalTitle: '置顶',
	        ...
	        attentionStandingDefaultTitle: '一直保留在「等你处理」',
	        attentionStandingDefaultEnabledSubtitle: '每个会话都会一直保留，直到你移除它',
	        attentionStandingDefaultDisabledSubtitle: '逐个保留会话',
	        attentionStandingDefaultUnavailableSubtitle: '请先设置「等你处理的任务」放在哪里',
	        workingPlacementModeTitle: '进行中的任务',
	        workingPlacementModeSubtitle: '进行中的任务放在哪里',
	        workingPlacementModeOffTitle: '保留在正常位置',
	        workingPlacementModeOffSubtitle: '按原来的分组和排序',
	        workingPlacementModeGlobalTitle: '排在其后',
	        workingPlacementModeGlobalSubtitle: '放在「等你处理」下面',
	        workingPlacementModeWithinGroupsTitle: '移到当前分组顶部',
	        workingPlacementModeWithinGroupsSubtitle: '留在所在文件夹或工作区里',
	        ...
	        activeColorActivityAndAttentionSubtitle: '进行中和等你处理的任务用醒目颜色。',
      ...
      sessionGettingStarted: {
        ...
        manualDisclosure: {
          ...
          subtitle: {
              ...
              createSession: '点 + 新建任务，或在电脑终端里启动。',

              selectSession: '从左边选一个任务，查看它的进展和改动。',
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `正在确认这台电脑能在 ${relay} 上响应。`,
    blockedTitle: '设置已停止',
    blockedStatusFallback: '设置未能完成。你的账户和会话没有受到影响。',
    blockedServiceConflictStatus: '需要先处理这台电脑上的另一个后台服务。没有做任何更改——打开“详情”可以查看是哪一个。',
    blockedConsentDeclinedStatus: '设置在做出任何更改之前就停止了。这台电脑上原有的后台服务保持原样。',
    blockedPairingDeclinedStatus: '这台电脑没有获准连接。没有做任何更改。',
    ...
    canceledTitle: '设置已取消',
    canceledStatus: '没有留下未完成的部分。再次运行设置会从这里继续。',
    ...
    consentConfirm: '替换',
    consentKeep: '保留现有服务',
    relayMoveTitle: '将后台服务转移到此 Relay？',
    relayMoveBody: ({ relay }: { relay: string }) => `\u6b64\u7535\u8111\u7684\u540e\u53f0\u670d\u52a1\u6b63\u5728\u53e6\u4e00\u4e2a Relay \u4e0a\u8fd0\u884c\u3002\u8f6c\u79fb\u5230 ${relay}\uff1f`,
    relayMoveConfirm: '交给它管理',
    relayMoveKeep: '保持现状',
    relayMoveAlways: '始终转移',
