  ...
  settings: {
    ...
    settingsSourceControl: {
        ...
        diffMode: {
		    ...
		    settingsDesktop: {
		        title: '桌面',
		        footer: '控制 Happier 在這台電腦上的行為。',
		        startOnLoginTitle: '登入時啟動',
		        startOnLoginSubtitle: '當你登入這台電腦時自動啟動 Happier。',
		        backgroundServiceTitle: '在背景保持可連線',
		        backgroundServiceSubtitle: '關閉 Happier 後，這台電腦仍會繼續回應。關掉之後，你的手機和瀏覽器在你再次開啟應用程式之前都無法連到這台電腦。',
		        backgroundServiceUnknown: '這台電腦上的 Happier 命令列沒有回報背景服務是否在登入時啟動。',
		        closeStopTitle: '代理程式工作階段仍在執行',
		        closeStopBody: '關閉 Happier 會停止這台電腦上的背景服務，並結束在這裡執行的工作階段。',
		        closeStopConfirm: '仍要停止',
		        closeStopKeep: '讓它繼續執行',
		    },
    ...
    settingsFeatures: {
        ...
        sessionListGrouping: {
            ...
            flatTitle: '不分組',
            flatSubtitle: '所有工作階段在一個列表中，不顯示分組標題',
        },
  ...
  newSession: {
    ...
    server: {
        ...
        relayDrift: {
            ...
            bannerAccountMismatchTitle: '你的背景服務登入的是另一個帳號',
            bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
                `應用程式正在使用 ${activeRelayUrl}，但這台電腦的背景服務登入的是另一個帳號。`,
    ...
    sessionsList: {
        ...
        moveSheetEmpty: '沒有可移動到的位置',
        moveSheetDestinations: '可選位置',
        moveSheetDisabledDescendant: '無法移動到自身或其子資料夾中。',
        moveSheetDisabledMaxDepth: '這會超出資料夾層級上限。',
        moveSheetDisabledCurrent: '已在此位置。',
        moveSheetDisabledUnavailable: '此位置無法使用。',
        dragHandleA11yLabel: '拖曳把手',
        dragA11yPickedUp: ({ item }: { item: string }) => `已拿起 ${item}。`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `已將 ${item} 移到 ${destination} 旁邊。`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `已將 ${item} 移入 ${destination}。`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `已將 ${item} 移到 ${destination}。`,
        dragA11yCancelled: ({ item }: { item: string }) => `已取消移動 ${item}。`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `無法移動 ${item}：${reason}`,
        dragA11yBlockedDescendantCycle: '目標位於被移動的資料夾內',
        dragA11yBlockedLeafCannotBeParent: '工作階段不能包含其他項目',
        dragA11yBlockedMaxDepth: '已達到資料夾層級上限',
        dragA11yBlockedSamePosition: '已在該位置',
        dragA11yBlockedWorkspaceScope: '目標位於其他工作區',
        dragA11yBlockedNoTarget: '未選擇目標',
        dragA11yBlockedDirectSession: '直連工作階段不能移入資料夾',
        dragA11yBlockedFeatureDisabled: '未啟用工作階段資料夾',
        dragA11yBlockedUnsupportedItem: '此項目不能移入資料夾',
        ...
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `成功 ${succeeded} 個，失敗 ${failed} 個，略過 ${skipped} 個`,
        selectionDismissResultA11yLabel: '關閉所選工作階段操作結果',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `要${action}已選取的 ${count} 個工作階段嗎？`,
...
settingsSession: {
    ...
    setupSurface: {
        ...
        stagePrepareStatus: '正在這台電腦上準備 Happier 命令列。',
        stageConnectStatus: ({ relay }: { relay: string }) => `正在把這台電腦連線到 ${relay}。你不需要做任何事。`,
        stageServiceStatus: '正在安裝讓工作階段保持可達的背景服務。',
        stageVerifyStatus: ({ relay }: { relay: string }) => `正在確認這台電腦能在 ${relay} 上回應。`,
        blockedTitle: '設定已停止',
        blockedStatusFallback: '設定未能完成。你的帳號與工作階段沒有受到影響。',
        blockedServiceConflictStatus: '需要先處理這台電腦上的另一個背景服務。沒有做任何變更——打開「詳細資料」可以看到是哪一個。',
        blockedConsentDeclinedStatus: '設定在做出任何變更之前就停止了。這台電腦上原有的背景服務維持原狀。',
        blockedPairingDeclinedStatus: '這台電腦沒有獲准連線。沒有做任何變更。',
        ...
        canceledTitle: '設定已取消',
        canceledStatus: '沒有留下未完成的部分。再次執行設定會從這裡繼續。',
        stepOfTotal: ({ step, total }: { step: number; total: number }) => `第 ${step} 步，共 ${total} 步`,
        hideDetails: '隱藏詳細資料',
        consentTitle: '要取代這台電腦上的背景服務嗎？',
        consentBodyFallback: '這台電腦上已經有一個不由本應用程式管理的 Happier 背景服務。',
        consentServicesList: ({ services }: { services: string }) => `受影響的服務：${services}`,
        consentConfirm: '取代',
        consentKeep: '保留現有服務',
        relayMoveTitle: '將背景服務轉移至此 Relay？',
        relayMoveBody: ({ relay }: { relay: string }) => `\u6b64\u96fb\u8166\u7684\u80cc\u666f\u670d\u52d9\u6b63\u5728\u53e6\u4e00\u500b Relay \u4e0a\u57f7\u884c\u3002\u8f49\u79fb\u81f3 ${relay}\uff1f`,
        relayMoveConfirm: '交給它管理',
        relayMoveKeep: '維持現狀',
        relayMoveAlways: '始終轉移',
        cliTrustTitle: '要核准此命令列嗎？',
        cliTrustBody: ({ command }: { command: string }) => `${command} 處的命令列不是 Happier 安裝的。核准後它可以讀寫此帳戶的對話。請僅核准你自己放在那裡的程式。`,
        cliTrustBodyUnknownCommand: '此命令列不是 Happier 安裝的。核准後它可以讀寫此帳戶的對話。請僅核准你自己放在那裡的程式。',
        cliTrustApprove: '核准',
    },
    ...
    machine: {
        ...
        cliPath: {
            title: '終端機',
            footer: 'Happier Desktop 只會新增或移除它自己建立的 PATH 項目；由 shell 安裝程式寫入的項目保持不變。',
            ...
            added: '已加入。開啟新的終端機即可使用 happier。',
            alreadyPresent: 'happier 已在你的 PATH 中。',
            removed: '已移除 Happier Desktop 新增的 PATH 項目。',
            nothingToRemove: 'Happier Desktop 未新增任何 PATH 項目。',
        },
        stopDaemon: '停止守護程序',
        replacementRepair: {
