...
export const es: TranslationStructure = {
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "Ninguna",
      flatSubtitle: "Todas las sesiones en una lista, sin encabezados de grupo",
    },
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: 'El servicio en segundo plano tiene la sesión iniciada con otra cuenta',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `La aplicación usa ${activeRelayUrl}, pero el servicio en segundo plano de este equipo tiene la sesión iniciada con otra cuenta.`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: 'Raíz del espacio de trabajo',
        ...
        moveSheetSearchPlaceholder: 'Buscar carpetas…',
        moveSheetEmpty: 'No hay destinos disponibles',
        moveSheetDestinations: 'Destinos',
        moveSheetDisabledDescendant: 'No se puede mover dentro de sí mismo ni de una subcarpeta.',
        moveSheetDisabledMaxDepth: 'Se superaría el límite de profundidad de carpetas.',
        moveSheetDisabledCurrent: 'Ya está en esta ubicación.',
        moveSheetDisabledUnavailable: 'Este destino no está disponible.',
        dragHandleA11yLabel: 'Asa de arrastre',
        dragA11yPickedUp: ({ item }: { item: string }) => `Se tomó ${item}.`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `Se movió ${item} junto a ${destination}.`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `Se movió ${item} dentro de ${destination}.`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `Se movió ${item} a ${destination}.`,
        dragA11yCancelled: ({ item }: { item: string }) => `Se canceló el movimiento de ${item}.`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `No se pudo mover ${item}: ${reason}`,
        ...
        dragA11yBlockedMaxDepth: 'se alcanzó el límite de profundidad de carpetas',
        dragA11yBlockedSamePosition: 'ya está en esa posición',
        dragA11yBlockedWorkspaceScope: 'el destino está en otro espacio de trabajo',
        dragA11yBlockedNoTarget: 'no se seleccionó ningún destino',
        dragA11yBlockedDirectSession: 'las sesiones directas no se pueden mover a carpetas',
        dragA11yBlockedFeatureDisabled: 'las carpetas de sesiones no están activadas',
        ...
        selectionSelectedCount: ({ count }: { count: number }) => count === 1 ? '1 sesión seleccionada' : `${count} sesiones seleccionadas`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => count === 1 ? '1 sesión seleccionada' : `${count} sesiones seleccionadas`,
        ...
        selectionProgress: ({ completed, total }: { completed: number; total: number }) => `${completed} de ${total} completadas`,
        selectionCancelRunningA11yLabel: 'Cancelar la acción sobre las sesiones seleccionadas',
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `${succeeded} correctas, ${failed} fallidas, ${skipped} omitidas`,
        selectionDismissResultA11yLabel: 'Descartar el resultado de la acción sobre las sesiones seleccionadas',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `¿${action} ${count} ${count === 1 ? 'sesión seleccionada' : 'sesiones seleccionadas'}?`,
        selectionConfirmA11yLabel: ({ action }: { action: string }) => `Confirmar ${action}`,
        selectionMoveSheetSourceLabel: ({ count }: { count: number }) => `${count} ${count === 1 ? 'sesión seleccionada' : 'sesiones seleccionadas'}`,
        orderingMode: {
            ...
            updated: 'Ordenar por última actividad',
        },
    attentionSectionTitle: 'Requiere atención',
    activeSectionTitle: 'Activas',
    inactiveSectionTitle: 'Inactivas',
    pinnedSectionTitle: 'Fijadas',
    reminders: {
...
settingsSession: {
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `Comprobando que este ordenador responde en ${relay}.`,
    blockedTitle: 'Configuración detenida',
    blockedStatusFallback: 'La configuración no pudo terminar. Tu cuenta y tus sesiones siguen intactas.',
    blockedServiceConflictStatus: 'Primero hay que resolver otro servicio en segundo plano de este ordenador. No se cambió nada: abre Detalles para ver cuál es.',
    blockedConsentDeclinedStatus: 'La configuración se detuvo antes de cambiar nada. El servicio en segundo plano que ya había en este ordenador se dejó tal cual.',
    blockedPairingDeclinedStatus: 'Este ordenador no se aprobó para conectarse. No se cambió nada.',
    ...
    canceledStatus: 'No quedó nada a medias. Al ejecutar la configuración de nuevo se continúa desde aquí.',
    ...
    relayMoveTitle: '¿Mover el servicio en segundo plano a este Relay?',
    relayMoveBody: ({ relay }: { relay: string }) => `El servicio en segundo plano de este ordenador se ejecuta en otro Relay. \u00bfMoverlo a ${relay}?`,
    relayMoveConfirm: 'Moverlo',
    relayMoveKeep: 'Dejarlo como está',
    relayMoveAlways: 'Moverlo siempre',
