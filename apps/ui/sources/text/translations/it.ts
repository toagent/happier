...
export const it: TranslationStructure = {
  ...
  taskStatus: {
    ready: "Pronto",
    working: "In corso…",
    permissionRequired: "Da approvare",
    actionRequired: "Attende risposta",
    failed: "Non riuscito",
    readyForReview: "Da rivedere",
    paused: "In pausa · invia per continuare",
    machineOffline: "Computer offline · in coda",
  },
  ...
  settingsSourceControl: {
    codeServer: {
      title: 'Revisione in code-server', footer: 'Apre solo le cartelle della macchina selezionata nella radice consentita.',
      machine: 'Macchina di revisione', url: 'Indirizzo HTTPS', root: 'Cartella radice consentita', unset: 'Non impostato',
      remove: 'Rimuovi configurazione', open: 'Apri in code-server', openFailed: 'Impossibile aprire code-server.',
      invalidUrl: 'Inserisci un indirizzo HTTPS senza credenziali né parametri di query.',
      invalidRoot: 'Inserisci un percorso assoluto diverso dalla radice del file system.',
    },
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "Nessuno",
      flatSubtitle: "Tutte le sessioni in un unico elenco, senza intestazioni di gruppo",
    },
  ...
  newSession: {
    ...
    executionNode: "Nodo di esecuzione",
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: 'Il tuo servizio in background ha eseguito l’accesso con un altro account',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `L’app sta usando ${activeRelayUrl}, ma il servizio in background di questo computer ha eseguito l’accesso con un altro account.`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: "Radice dell'area di lavoro",
        ...
        moveSheetTitle: ({ item }: { item: string }) => `Sposta ${item}`,
        moveSheetDestinationLabel: 'Destinazione',
        moveSheetSubmit: 'Sposta',
        moveSheetSearchPlaceholder: 'Cerca cartelle…',
        moveSheetEmpty: 'Nessuna destinazione disponibile',
        moveSheetDestinations: 'Destinazioni',
        moveSheetDisabledDescendant: 'Impossibile spostare in sé stesso o in una sottocartella.',
        moveSheetDisabledMaxDepth: 'Si supererebbe il limite di profondità delle cartelle.',
        ...
        dragA11yPickedUp: ({ item }: { item: string }) => `Preso ${item}.`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `${item} spostato vicino a ${destination}.`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `${item} spostato in ${destination}.`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `${item} spostato a ${destination}.`,
        dragA11yCancelled: ({ item }: { item: string }) => `Spostamento annullato per ${item}.`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `Impossibile spostare ${item}: ${reason}`,
        dragA11yBlockedDescendantCycle: 'la destinazione è dentro la cartella spostata',
        dragA11yBlockedLeafCannotBeParent: 'le sessioni non possono contenere altri elementi',
        dragA11yBlockedMaxDepth: 'limite di profondità delle cartelle raggiunto',
        dragA11yBlockedSamePosition: 'già in quella posizione',
        dragA11yBlockedWorkspaceScope: 'la destinazione è in un’altra area di lavoro',
        ...
        dragA11yBlockedFeatureDisabled: 'le cartelle delle sessioni non sono attive',
        ...
        selectionSelectedCount: ({ count }: { count: number }) => count === 1 ? '1 sessione selezionata' : `${count} sessioni selezionate`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => count === 1 ? '1 sessione selezionata' : `${count} sessioni selezionate`,
        ...
        selectionProgress: ({ completed, total }: { completed: number; total: number }) => `${completed} di ${total} completate`,
        selectionCancelRunningA11yLabel: 'Annulla l\'azione sulle sessioni selezionate',
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `${succeeded} riuscite, ${failed} non riuscite, ${skipped} saltate`,
        selectionDismissResultA11yLabel: 'Chiudi il risultato dell\'azione sulle sessioni selezionate',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `${action} ${count} ${count === 1 ? 'sessione selezionata' : 'sessioni selezionate'}?`,
        selectionConfirmA11yLabel: ({ action }: { action: string }) => `Conferma ${action}`,
        selectionMoveSheetSourceLabel: ({ count }: { count: number }) => `${count} ${count === 1 ? 'sessione selezionata' : 'sessioni selezionate'}`,
        orderingMode: {
            ...
            created: 'Ordina per data di creazione',
            updated: 'Ordina per ultima attività',
        },
    attentionSectionTitle: 'Richiede attenzione',
    activeSectionTitle: 'Attive',
    inactiveSectionTitle: 'Inattive',
    pinnedSectionTitle: 'Fissate',
    reminders: {
...
settingsSession: {
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `Verifica che questo computer risponda su ${relay}.`,
    blockedTitle: 'Configurazione interrotta',
    blockedStatusFallback: 'La configurazione non è riuscita a completarsi. Il tuo account e le tue sessioni sono intatti.',
    blockedServiceConflictStatus: 'Prima bisogna risolvere un altro servizio in background su questo computer. Non è stato cambiato nulla: apri Dettagli per vedere quale.',
    blockedConsentDeclinedStatus: 'La configurazione si è fermata prima di cambiare qualcosa. Il servizio in background già presente è rimasto esattamente com’era.',
    blockedPairingDeclinedStatus: 'Questo computer non è stato approvato per la connessione. Non è stato cambiato nulla.',
    ...
    canceledStatus: 'Non è rimasto nulla a metà. Rieseguendo la configurazione si riparte da qui.',
    ...
    relayMoveTitle: 'Spostare il servizio in background su questo Relay?',
    relayMoveBody: ({ relay }: { relay: string }) => `Il servizio in background di questo computer \u00e8 su un altro Relay. Spostarlo su ${relay}?`,
    relayMoveConfirm: 'Spostalo',
    relayMoveKeep: 'Lascialo com’è',
    relayMoveAlways: 'Sposta sempre',
