...
export const pl: TranslationStructure = {
  ...
  taskStatus: {
    ready: "Gotowe",
    working: "W toku…",
    permissionRequired: "Do zatwierdzenia",
    actionRequired: "Czeka na odpowiedź",
    failed: "Błąd",
    readyForReview: "Do przejrzenia",
    paused: "Wstrzymane · wyślij, aby kontynuować",
    machineOffline: "Komputer offline · w kolejce",
  },
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "Brak",
      flatSubtitle: "Wszystkie sesje na jednej liście, bez nagłówków grup",
    },
  ...
  session: {
    ...
    changesStatusAction: ({ added, removed }: { added: number; removed: number }) =>
        `Zobacz zmiany: dodano ${added} wierszy, usunięto ${removed}`,
    serverLink: {
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: 'Usługa w tle jest zalogowana na innym koncie',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `Aplikacja używa ${activeRelayUrl}, ale usługa w tle na tym komputerze jest zalogowana na innym koncie.`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: 'Katalog główny przestrzeni roboczej',
        ...
        moveSheetTitle: ({ item }: { item: string }) => `Przenieś ${item}`,
        moveSheetDestinationLabel: 'Miejsce docelowe',
        moveSheetSubmit: 'Przenieś',
        moveSheetSearchPlaceholder: 'Szukaj folderów…',
        moveSheetEmpty: 'Brak dostępnych miejsc docelowych',
        moveSheetDestinations: 'Miejsca docelowe',
        moveSheetDisabledDescendant: 'Nie można przenieść do samego siebie ani do podfolderu.',
        moveSheetDisabledMaxDepth: 'Spowodowałoby to przekroczenie limitu zagnieżdżenia folderów.',
        moveSheetDisabledCurrent: 'Już w tym miejscu.',
        moveSheetDisabledUnavailable: 'To miejsce docelowe jest niedostępne.',
        dragHandleA11yLabel: 'Uchwyt przeciągania',
        dragA11yPickedUp: ({ item }: { item: string }) => `Podniesiono ${item}.`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `Przeniesiono ${item} obok ${destination}.`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `Przeniesiono ${item} do ${destination}.`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `Przeniesiono ${item} na ${destination}.`,
        dragA11yCancelled: ({ item }: { item: string }) => `Anulowano przenoszenie ${item}.`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `Nie można przenieść ${item}: ${reason}`,
        dragA11yBlockedDescendantCycle: 'miejsce docelowe jest wewnątrz przenoszonego folderu',
        dragA11yBlockedLeafCannotBeParent: 'sesje nie mogą zawierać innych elementów',
        dragA11yBlockedMaxDepth: 'osiągnięto limit zagnieżdżenia folderów',
        dragA11yBlockedSamePosition: 'już w tej pozycji',
        dragA11yBlockedWorkspaceScope: 'miejsce docelowe jest w innej przestrzeni roboczej',
        ...
        selectionSelectedCount: ({ count }: { count: number }) => `Wybrane sesje: ${count}`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => `Wybrane sesje: ${count}`,
        ...
        selectionProgress: ({ completed, total }: { completed: number; total: number }) => `Ukończono ${completed} z ${total}`,
        selectionCancelRunningA11yLabel: 'Anuluj działanie na zaznaczonych sesjach',
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `Udane: ${succeeded}, nieudane: ${failed}, pominięte: ${skipped}`,
        selectionDismissResultA11yLabel: 'Zamknij wynik działania na zaznaczonych sesjach',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `${action} — wybrane sesje: ${count}?`,
        selectionConfirmA11yLabel: ({ action }: { action: string }) => `Potwierdź: ${action}`,
        selectionMoveSheetSourceLabel: ({ count }: { count: number }) => `Wybrane sesje: ${count}`,
        orderingMode: {
            title: 'Kolejność sesji',
            description: 'Wybierz ręczną kolejność albo stabilne sortowanie według daty.',
            custom: 'Własna kolejność',
            created: 'Sortuj według daty utworzenia',
            updated: 'Sortuj według ostatniej aktywności',
        },
    attentionSectionTitle: 'Wymaga uwagi',
    activeSectionTitle: 'Aktywne',
    inactiveSectionTitle: 'Nieaktywne',
    pinnedSectionTitle: 'Przypięte',
    reminders: {
  ...
  directSessions: {
    ...
    deleteCandidateActionsAccessibilityLabel: ({ title }: { title: string }) => `Więcej działań dla ${title}`,
    deleteCandidateConfirmTitle: "Usunąć sesję dostawcy?",
    deleteCandidateConfirmMessage: ({ title, provider }: { title: string; provider: string }) => `Usunąć „${title}” z ${provider}? Tej operacji nie można cofnąć.`,
    deleteCandidateFailed: "Nie udało się usunąć sesji dostawcy.",
  },
...
settingsSession: {
        ...
        freshSubtitle: 'Otwieraj nową wersję roboczą za każdym wybraniem Nowej sesji.',
	      ...
	      sessionList: {
           ...
           agentActivityCountDisabledSubtitle: 'Nie pokazuj liczby agentów w wierszach',
	          ...
	          storageSourceTitle: 'Źródło sesji',
	          storageSourceSubtitle: 'Sesje Happier lub sesje bezpośrednie na Twoich maszynach',
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `Sprawdzamy, czy ten komputer odpowiada na ${relay}.`,
    blockedTitle: 'Konfiguracja zatrzymana',
    blockedStatusFallback: 'Konfiguracja nie mogła się zakończyć. Twoje konto i sesje pozostają nienaruszone.',
    blockedServiceConflictStatus: 'Najpierw trzeba rozwiązać sprawę innej usługi w tle na tym komputerze. Nic nie zostało zmienione — otwórz Szczegóły, aby zobaczyć którą.',
    blockedConsentDeclinedStatus: 'Konfiguracja zatrzymała się, zanim cokolwiek zmieniła. Usługa w tle, która już tu była, została dokładnie taka, jaka była.',
    blockedPairingDeclinedStatus: 'Ten komputer nie został zatwierdzony do połączenia. Nic nie zostało zmienione.',
    ...
    canceledStatus: 'Nic nie zostało zrobione połowicznie. Ponowne uruchomienie konfiguracji kontynuuje od tego miejsca.',
    ...
    relayMoveTitle: 'Przenieść usługę w tle na ten Relay?',
    relayMoveBody: ({ relay }: { relay: string }) => `Us\u0142uga w tle na tym komputerze dzia\u0142a na innym Relay. Przenie\u015b\u0107 j\u0105 na ${relay}?`,
    relayMoveConfirm: 'Przenieś',
    relayMoveKeep: 'Zostaw bez zmian',
    relayMoveAlways: 'Zawsze przenoś',
