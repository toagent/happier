...
localTailscale: {
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "グループなし",
      flatSubtitle: "すべてのセッションをグループ見出しなしで1つのリストに表示",
    },
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: 'バックグラウンドサービスは別のアカウントでサインインしています',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `アプリは ${activeRelayUrl} を使用していますが、このコンピューターのバックグラウンドサービスは別のアカウントでサインインしています。`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: 'ワークスペースのルート',
        ...
        moveSheetTitle: ({ item }: { item: string }) => `${item} を移動`,
        moveSheetDestinationLabel: '移動先',
        moveSheetSubmit: '移動',
        moveSheetSearchPlaceholder: 'フォルダーを検索…',
        moveSheetEmpty: '移動先がありません',
        moveSheetDestinations: '移動先の一覧',
        moveSheetDisabledDescendant: '自身またはその子フォルダーには移動できません。',
        moveSheetDisabledMaxDepth: 'フォルダーの階層上限を超えます。',
        moveSheetDisabledCurrent: 'すでにこの場所にあります。',
        moveSheetDisabledUnavailable: 'この移動先は利用できません。',
        dragHandleA11yLabel: 'ドラッグハンドル',
        dragA11yPickedUp: ({ item }: { item: string }) => `${item} を持ち上げました。`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `${item} を ${destination} の近くに移動しました。`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `${item} を ${destination} に入れました。`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `${item} を ${destination} に移動しました。`,
        dragA11yCancelled: ({ item }: { item: string }) => `${item} の移動をキャンセルしました。`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `${item} を移動できませんでした: ${reason}`,
        dragA11yBlockedDescendantCycle: '移動先が移動中のフォルダー内にあります',
        dragA11yBlockedLeafCannotBeParent: 'セッションには他の項目を含められません',
        dragA11yBlockedMaxDepth: 'フォルダーの階層上限に達しました',
        dragA11yBlockedSamePosition: 'すでにその位置にあります',
        dragA11yBlockedWorkspaceScope: '移動先が別のワークスペースにあります',
        dragA11yBlockedNoTarget: '移動先が選択されていません',
        dragA11yBlockedDirectSession: 'ダイレクトセッションはフォルダーに移動できません',
        dragA11yBlockedFeatureDisabled: 'セッションフォルダーが有効になっていません',
        dragA11yBlockedUnsupportedItem: 'この項目はフォルダーに移動できません',
        dragA11yBlockedDateOrderingMode: 'セッションの順序は現在の日付順で制御されています。',
        selectionSelectedCount: ({ count }: { count: number }) => `${count} 件のセッションを選択中`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => `${count} 件のセッションを選択中`,
        ...
        selectionProgress: ({ completed, total }: { completed: number; total: number }) => `${completed}/${total} 完了`,
        selectionCancelRunningA11yLabel: '選択したセッションの操作をキャンセル',
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `成功 ${succeeded} 件、失敗 ${failed} 件、スキップ ${skipped} 件`,
        selectionDismissResultA11yLabel: '選択したセッション操作の結果を閉じる',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `選択した ${count} 件のセッションを${action}しますか？`,
        selectionConfirmA11yLabel: ({ action }: { action: string }) => `${action}を確認`,
        selectionMoveSheetSourceLabel: ({ count }: { count: number }) => `選択した ${count} 件のセッション`,
        orderingMode: {
            title: 'セッションの順序',
    ...
    activeSectionTitle: 'アクティブ',
    inactiveSectionTitle: '非アクティブ',
...
settingsSession: {
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `このコンピュータが ${relay} で応答するか確認しています。`,
    blockedTitle: 'セットアップが停止しました',
    blockedStatusFallback: 'セットアップを完了できませんでした。アカウントとセッションはそのままです。',
    blockedServiceConflictStatus: 'このコンピュータの別のバックグラウンドサービスを先に解決する必要があります。何も変更していません。詳細を開くとどれか確認できます。',
    blockedConsentDeclinedStatus: '何も変更する前にセットアップを停止しました。すでにあったバックグラウンドサービスはそのままです。',
    blockedPairingDeclinedStatus: 'このコンピュータは接続を承認されませんでした。何も変更していません。',
    ...
    canceledStatus: '中途半端な状態は残っていません。もう一度実行すればここから続きます。',
    ...
    relayMoveTitle: 'バックグラウンドサービスをこの Relay に移動しますか？',
    relayMoveBody: ({ relay }: { relay: string }) => `\u3053\u306e\u30b3\u30f3\u30d4\u30e5\u30fc\u30bf\u30fc\u306e\u30d0\u30c3\u30af\u30b0\u30e9\u30a6\u30f3\u30c9\u30b5\u30fc\u30d3\u30b9\u306f\u5225\u306e Relay \u3067\u5b9f\u884c\u4e2d\u3067\u3059\u3002${relay} \u306b\u79fb\u52d5\u3057\u307e\u3059\u304b\uff1f`,
    relayMoveConfirm: '移す',
    relayMoveKeep: 'このままにする',
    relayMoveAlways: '常に移動する',
