...
export const pt: TranslationStructure = {
  ...
  taskStatus: {
    ...
    permissionRequired: "Aguardando aprovação",
    actionRequired: "Aguardando resposta",
    failed: "Falhou",
    readyForReview: "Para revisar",
    paused: "Pausado · envie para continuar",
    machineOffline: "Computador offline · na fila",
  },
  ...
  settingsSourceControl: {
    codeServer: {
      title: 'Revisão no code-server', footer: 'Abra apenas diretórios da máquina selecionada dentro da raiz permitida.',
      machine: 'Máquina de revisão', url: 'Endereço HTTPS', root: 'Diretório raiz permitido', unset: 'Não definido',
      remove: 'Remover configuração', open: 'Abrir no code-server', openFailed: 'Não foi possível abrir o code-server.',
      invalidUrl: 'Insira um endereço HTTPS sem credenciais ou parâmetros de consulta.',
      invalidRoot: 'Insira um caminho absoluto diferente da raiz do sistema de arquivos.',
    },
  ...
  settingsFeatures: {
    ...
    sessionListGrouping: {
      ...
      flatTitle: "Nenhum",
      flatSubtitle: "Todas as sessões em uma lista, sem cabeçalhos de grupo",
    },
  ...
  session: {
    ...
    serverLink: {
        ...
        unreachable: "Sem conexão com o servidor · reconectando",
        authRequired: "Entre novamente para reconectar",
        error: "Erro do servidor",
        noticeTitle: "Este dispositivo não está conectado",
        noticeBody: "Este dispositivo não consegue alcançar o servidor agora e continua reconectando sozinho. A máquina e esta sessão continuam rodando; esta tela se atualiza quando a conexão voltar.",
    },
  ...
  server: {
    ...
    relayDrift: {
        ...
        bannerAccountMismatchTitle: 'Seu serviço em segundo plano está conectado com outra conta',
        bannerAccountMismatchDescription: ({ activeRelayUrl }: { activeRelayUrl: string }) =>
            `O app está usando ${activeRelayUrl}, mas o serviço em segundo plano deste computador está conectado com outra conta.`,
  ...
  sessionsList: {
    ...
    moveToWorkspaceRoot: 'Raiz da área de trabalho',
        ...
        moveSheetTitle: ({ item }: { item: string }) => `Mover ${item}`,
        moveSheetDestinationLabel: 'Destino',
        moveSheetSubmit: 'Mover',
        moveSheetSearchPlaceholder: 'Pesquisar pastas…',
        moveSheetEmpty: 'Nenhum destino disponível',
        moveSheetDestinations: 'Destinos',
        moveSheetDisabledDescendant: 'Não é possível mover para si mesmo ou para uma subpasta.',
        moveSheetDisabledMaxDepth: 'Isso excederia o limite de profundidade de pastas.',
        moveSheetDisabledCurrent: 'Já está neste local.',
        moveSheetDisabledUnavailable: 'Este destino não está disponível.',
        dragHandleA11yLabel: 'Alça de arrastar',
        dragA11yPickedUp: ({ item }: { item: string }) => `Pegou ${item}.`,
        dragA11yDroppedReorder: ({ item, destination }: { item: string; destination: string }) => `${item} movido para perto de ${destination}.`,
        dragA11yDroppedNest: ({ item, destination }: { item: string; destination: string }) => `${item} movido para dentro de ${destination}.`,
        dragA11yDroppedRoot: ({ item, destination }: { item: string; destination: string }) => `${item} movido para ${destination}.`,
        dragA11yCancelled: ({ item }: { item: string }) => `Movimento de ${item} cancelado.`,
        dragA11yBlocked: ({ item, reason }: { item: string; reason: string }) => `Não foi possível mover ${item}: ${reason}`,
        ...
        dragA11yBlockedMaxDepth: 'limite de profundidade de pastas atingido',
        dragA11yBlockedSamePosition: 'já está nessa posição',
        dragA11yBlockedWorkspaceScope: 'o destino está em outra área de trabalho',
        ...
        dragA11yBlockedFeatureDisabled: 'pastas de sessões não estão ativadas',
        ...
        selectionSelectedCount: ({ count }: { count: number }) => count === 1 ? '1 sessão selecionada' : `${count} sessões selecionadas`,
        selectionA11ySelectedCount: ({ count }: { count: number }) => count === 1 ? '1 sessão selecionada' : `${count} sessões selecionadas`,
        ...
        selectionProgress: ({ completed, total }: { completed: number; total: number }) => `${completed} de ${total} concluídas`,
        selectionCancelRunningA11yLabel: 'Cancelar ação nas sessões selecionadas',
        selectionResult: ({ succeeded, failed, skipped }: { succeeded: number; failed: number; skipped: number }) => `${succeeded} com sucesso, ${failed} com falha, ${skipped} ignoradas`,
        selectionDismissResultA11yLabel: 'Dispensar resultado da ação nas sessões selecionadas',
        selectionConfirm: ({ action, count }: { action: string; count: number }) => `${action} ${count} ${count === 1 ? 'sessão selecionada' : 'sessões selecionadas'}?`,
        selectionConfirmA11yLabel: ({ action }: { action: string }) => `Confirmar ${action}`,
        selectionMoveSheetSourceLabel: ({ count }: { count: number }) => `${count} ${count === 1 ? 'sessão selecionada' : 'sessões selecionadas'}`,
        orderingMode: {
            ...
            created: 'Ordenar por data de criação',
            updated: 'Ordenar pela última atividade',
        },
    attentionSectionTitle: 'Precisa de atenção',
    activeSectionTitle: 'Ativas',
    inactiveSectionTitle: 'Inativas',
    pinnedSectionTitle: 'Fixadas',
    reminders: {
...
settingsSession: {
  ...
  setupSurface: {
    ...
    stageVerifyStatus: ({ relay }: { relay: string }) => `Verificando se este computador responde em ${relay}.`,
    blockedTitle: 'Configuração interrompida',
    blockedStatusFallback: 'A configuração não pôde terminar. Sua conta e suas sessões continuam intactas.',
    blockedServiceConflictStatus: 'Primeiro é preciso resolver outro serviço em segundo plano deste computador. Nada foi alterado — abra Detalhes para ver qual.',
    blockedConsentDeclinedStatus: 'A configuração parou antes de alterar qualquer coisa. O serviço em segundo plano que já existia ficou exatamente como estava.',
    blockedPairingDeclinedStatus: 'Este computador não foi aprovado para se conectar. Nada foi alterado.',
    ...
    blockedCliUnavailableStatus: 'O Happier não conseguiu iniciar a sua linha de comando neste computador. Nada foi alterado.',
    ...
    canceledStatus: 'Nada ficou pela metade. Executar a configuração de novo continua daqui.',
    ...
    relayMoveTitle: 'Mover o serviço em segundo plano para este Relay?',
    relayMoveBody: ({ relay }: { relay: string }) => `O servi\u00e7o em segundo plano deste computador est\u00e1 noutro Relay. Mov\u00ea-lo para ${relay}?`,
    relayMoveConfirm: 'Mover',
    relayMoveKeep: 'Deixar como está',
    relayMoveAlways: 'Mover sempre',
