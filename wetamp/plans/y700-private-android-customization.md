...
- 状态：APPROVED_EXECUTION；合约修订：4；日期：2026-09-24。
...
- 修订 3 的硬边界：三机共享一个 FIFO 且总并发最多 3；远端 worker 不直接写本机主 checkout；不使用 Unison、SSHFS 或持续双向同步主工作区；任务级审查副本只从执行结果单向物化；执行机器与审查机器只在任务详情中显示（修订 4）；基线失配时允许查看，但自动应用必须失败并明确标记 stale/conflict。
...
- 修订 4（2026-09-24，用户在会话中逐项决定后批准）：用户不再管理机器，交互只保留“派发任务”和“查看变更”。
  - 自动分派：新任务不选机器，由本机调度器决定执行 worker；规则为“本机优先，本机已有运行中的调度任务时外溢到空闲 worker”；总并发与 FIFO 上限不变。客户端只在本机声明支持自动分派时省略 worker，否则保持修订 3 的显式 worker 行为。
  - 机器信息：左栏与新建任务页不显示机器；任务详情仍显示执行机器与审查机器，用于排障。
  - 左栏：完全扁平的任务列表，不按活动状态、项目、机器或日期分段；已归档任务入口保留。
  - 取代：§4.1“新任务默认本机，选择机器”一句与 R2 中“执行机器身份明确”的显示要求（身份仍记录在任务 metadata 并在详情可查）。
...
| R2 | 三机各三种 Agent 可由本机自动分派（修订 4），执行机器/项目/Agent 身份记录在任务并可在详情查看 | 九条目标路径实际执行、追问、权限处理与停止；不能将九个入口显示当通过 |
...
- 新任务不选机器（修订 4：本机调度器自动分派），选择项目目录、Claude/Codex/OpenCode；常用项目置顶，提供“实现、评审、测试”等输入模板，模板不隐藏权限和目标目录。
